function output = simulate_with_growth_and_bead(params,reaction_fn,t_save, bead_params)
...
%setup bead perturbation
bead = zeros(N,numvar);
bead_pos = round(bead_params.location / dL);
bead_half_width = round(0.5*bead_params.width / dL);
bead((bead_pos-bead_half_width):((bead_pos+bead_half_width)),bead_params.species) = bead_params.amplitude;
